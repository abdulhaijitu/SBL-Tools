            ...
            <div data-presentation-id="{{ $pres->id }}" class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 hover:border-orange-200 transition-all flex flex-col justify-between">
                ...
                <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        @can('presentations.manage')<form action="/presentations/{{ $pres->id }}" method="POST" onsubmit="return confirm('Delete presentation record?')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-1.5 text-slate-400 hover:text-rose-600 rounded-md hover:bg-rose-50 transition-colors" title="Delete Presentation">
