...
<div class="space-y-6" x-data="abbreviationManager" data-terms="{{ json_encode($abbreviations) }}" data-can-manage="{{ auth()->user()?->hasPermission('users.manage') ? '1' : '0' }}">
    ...
    <div class="flex gap-2 overflow-x-auto pb-2">
        ...
        <button type="button" class="btn-secondary whitespace-nowrap" :aria-pressed="category === '{{ $cat['slug'] }}'" @click="category = '{{ $cat['slug'] }}'">{{ $cat['icon'] }} {{ $cat['name'] }} (<span x-text="count('{{ $cat['slug'] }}')"></span>)</button>
        @endforeach
    </div>
    <label class="block"><span class="text-sm font-semibold">Search abbreviations</span><input class="mt-1 w-full rounded-xl border-slate-300" type="search" x-model="search" placeholder="Search short form, name, or Bangla meaning"></label>
    <section x-show="editing" x-cloak class="bg-white border border-slate-200 rounded-2xl p-5" aria-label="Abbreviation editor">
        ...
        <form @submit.prevent="save()" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                @foreach(['code' => ['Short Form', 50], 'name' => ['Full Name', 200], 'meaning_bn' => ['বাংলা অর্থ (Meaning)', 1000], 'description_bn' => ['ব্যবহার (Use Case)', 3000], 'icon' => ['Icon (optional)', 20], 'tag' => ['Tag (optional)', 100]] as $field => [$label, $max])
    ...
    <div class="bg-white rounded-2xl border border-slate-200 overflow-x-auto">
        <table class="w-full text-left text-sm">
            <thead class="bg-slate-50"><tr>@foreach(['Short Form', 'Full Name', 'Category', 'বাংলা অর্থ', 'Use Case', 'Actions'] as $heading)<th class="p-4">{{ $heading }}</th>@endforeach</tr></thead>
            <tbody class="divide-y divide-slate-100">
                <template x-for="term in filtered" :key="term.id"><tr>
                    ...
                    <td class="p-4" x-text="term.category"></td><td class="p-4" x-text="term.meaning_bn"></td><td class="p-4" x-text="term.description_bn"></td>
