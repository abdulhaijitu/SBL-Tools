...
<div class="space-y-6">
    ...
    <div class="section-heading">
        <div><p id="dashboard-current-date">{{ now()->format('l, j F Y') }}</p><h2>Welcome back, {{ Auth::user()->name }}</h2><p>Your leads, follow-ups and team at a glance.</p></div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
        ...
        <a href="{{ route('leads.index', ['filter' => 'due_today']) }}" class="metric-card"><span>Today Followup</span><strong id="today-followup">{{ $followupsDueToday->count() }}</strong><small>Lead touchpoints due</small></a>
        <a href="{{ route('tasks.index', ['filter' => 'today']) }}" class="metric-card"><span>Today's Tasks</span><strong id="today-tasks-kpi">{{ $todayTasks->count() }}</strong><small>Pending action items</small></a>
    ...
    @can('leads.view')
    <!-- 2. Action Priorities: Overdue Follow-ups & Today's Scheduled Tasks (Most Prominent) -->
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden flex flex-col">
            ...
            <!-- Empty State Placeholder (Always present in DOM) -->
            <div id="dashboard-overdue-empty" class="p-8 text-center text-slate-400 text-xs my-auto" @if($overdueFollowups->isNotEmpty()) style="display: none;" @endif>
                <span class="text-2xl block mb-1">🎉</span>
                <span class="font-semibold text-slate-600">Great job! No overdue follow-ups right now.</span>
            </div>

            <!-- Overdue List Container (Always present in DOM) -->
            <div id="dashboard-overdue-container" class="divide-y divide-slate-100" @if($overdueFollowups->isEmpty()) style="display: none;" @endif>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden flex flex-col">
            ...
            <!-- Empty State Placeholder (Always present in DOM) -->
            <div id="dashboard-tasks-empty" class="p-8 text-center text-slate-400 text-xs my-auto" @if($todayTasks->isNotEmpty()) style="display: none;" @endif>
                <span class="text-2xl block mb-1">📅</span>
                <span class="font-semibold text-slate-600">No pending tasks scheduled for today yet.</span>
            </div>

            <!-- Tasks List Container (Always present in DOM) -->
            <div id="dashboard-tasks-container" class="divide-y divide-slate-100" @if($todayTasks->isEmpty()) style="display: none;" @endif>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-xs">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div>
                ...
                <p class="text-xs text-slate-500">Distribution of active leads across sales lifecycle stages</p>
            </div>
    ...
    <!-- 4. Priorities & Recent Activity Stream (3-Column Split) -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">

        <!-- High Priority Leads (Hot Leads) -->
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-col">
            ...
            <div id="dashboard-hot-empty" class="py-6 text-center text-slate-400 text-xs my-auto" @if($hotLeads->isNotEmpty()) style="display: none;" @endif>
                No leads scored as Hot (80+) yet.
            </div>

            <div id="dashboard-hot-container" class="space-y-2.5" @if($hotLeads->isEmpty()) style="display: none;" @endif>
        ...
        <!-- Stale Leads Alert -->
        <div class="bg-white rounded-2xl border border-purple-200 shadow-xs p-5 bg-purple-50/10 flex flex-col">
            ...
            <div id="dashboard-stale-empty" class="py-6 text-center text-purple-400 text-xs my-auto" @if($staleLeads->isNotEmpty()) style="display: none;" @endif>
                No stale leads requiring re-engagement.
            </div>

            <div id="dashboard-stale-container" class="space-y-2" @if($staleLeads->isEmpty()) style="display: none;" @endif>
                @foreach ($staleLeads as $lead)
                    <div data-lead-id="{{ $lead->id }}" class="flex items-center justify-between p-2.5 bg-white border border-purple-100 rounded-xl text-xs shadow-2xs">
                        <div class="min-w-0 pr-2">
                            ...
                            <div class="text-[10px] text-slate-400 mt-0.5">📞 {{ $lead->mobile }} • Last contact: {{ $lead->last_contact_at?->diffForHumans() ?? 'None' }}</div>
                        </div>
                        <a href="{{ route('leads.show', $lead->id) }}" class="text-[11px] font-bold text-purple-700 hover:underline flex-shrink-0">
                            Re-engage →
                        </a>
                    </div>
                @endforeach
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-col md:col-span-2 lg:col-span-1">
            <div class="flex items-center justify-between mb-3 pb-2 border-b border-slate-100">
                <h3 class="text-sm font-bold text-slate-900 flex items-center gap-1.5">
