...
export const rolePermissions = sqliteTable(
    ...
    (table) => [
        uniqueIndex("idx_role_permissions_unique").on(
            table.roleId,
            table.permissionId,
        ),
    ],
...
export const presentations = sqliteTable("presentations", {
    ...
    leadId: integer("lead_id")
        .notNull()
        .references(() => leads.id, { onDelete: "cascade" }),
    presenterUserId: integer("presenter_user_id").references(() => users.id, {
        onDelete: "set null",
    }),
...
export const dailyActivityLogs = sqliteTable(
    ...
    (table) => [
        uniqueIndex("idx_daily_activity_user_date").on(
            table.userId,
            table.logDate,
        ),
    ],
...
export const investments = sqliteTable(
    ...
    (table) => [index("idx_investments_user").on(table.userId)],
);
...
export const binaryNodes = sqliteTable(
    ...
    (table) => [
        uniqueIndex("idx_binary_parent_position").on(
            table.parentId,
