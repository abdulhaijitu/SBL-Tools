    ...
    if (curPath === '/leads' || curPath.startsWith('/leads?')) {
      ...
      if (DATA.leads && DATA.leads.length > 0) {
        ...
        DATA.leads.slice().reverse().forEach(function(lead) {
          ...
          if (tableBody) {
            const rowHtml = '<td class="py-3.5 px-4"><div class="flex items-center gap-3"><div class="w-9 h-9 rounded-xl bg-orange-100 text-orange-700 font-bold flex items-center justify-center text-xs flex-shrink-0">' + initialLetter + '</div><div><a href="/leads/' + lead.id + '" class="font-bold text-slate-900 hover:text-orange-600 text-sm block">' + escapeHtml(lead.name) + '</a><div class="text-[11px] text-slate-400">' + escapeHtml(lead.location || 'No location') + (lead.profession_or_business ? ' • ' + escapeHtml(lead.profession_or_business) : '') + '</div></div></div></td><td class="py-3.5 px-4"><div class="font-medium text-slate-800">' + escapeHtml(lead.mobile) + '</div><div class="flex items-center gap-2 mt-1"><a href="tel:' + escapeHtml(lead.mobile) + '" title="Call" class="text-slate-400 hover:text-emerald-600 text-sm">📞</a><a href="https://wa.me/' + cleanWhatsapp + '" target="_blank" title="WhatsApp" class="text-slate-400 hover:text-emerald-600 text-sm">💬</a></div></td><td class="py-3.5 px-4"><span class="font-medium text-slate-800 block">' + escapeHtml(sourceName) + '</span><div class="flex flex-wrap gap-1 mt-1">' + interests.map(function(i) { return '<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-semibold bg-orange-50 text-orange-700 border border-orange-200/80">' + escapeHtml(i) + '</span>'; }).join('') + '</div></td><td class="py-3.5 px-4"><span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-orange-50 text-orange-700 border border-orange-200">' + stageLabel + '</span></td><td class="py-3.5 px-4">' + (lead.next_action_at ? ('<div class="text-xs font-medium text-slate-700">' + escapeHtml(lead.next_action_type || 'Action') + '<br><span class="text-[11px] text-slate-400">' + escapeHtml(lead.next_action_at) + '</span></div>') : '<span class="text-[11px] text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded-md">Needs Next Action</span>') + '</td><td class="py-3.5 px-4 text-right"><div class="flex items-center justify-end gap-1.5"><a href="/leads/' + lead.id + '" class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition-all active:scale-95 shadow-xs border border-slate-200/60" title="View Lead Profile" aria-label="View Lead Profile"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg><span class="sr-only">View</span></a><a href="/leads/' + lead.id + '/edit" class="w-8 h-8 rounded-lg flex items-center justify-center bg-orange-50 hover:bg-orange-100 text-orange-600 hover:text-orange-800 transition-all active:scale-95 shadow-xs border border-orange-200/60" title="Edit Lead" aria-label="Edit Lead"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg><span class="sr-only">Edit</span></a><form action="/leads/' + lead.id + '" method="POST" onsubmit="return confirm(&quot;Are you sure you want to delete this lead?&quot;);" class="inline"><input type="hidden" name="_method" value="DELETE"><button type="submit" class="w-8 h-8 rounded-lg flex items-center justify-center bg-rose-50 hover:bg-rose-100 text-rose-600 hover:text-rose-800 transition-all active:scale-95 shadow-xs border border-rose-200/60 cursor-pointer" title="Delete Lead" aria-label="Delete Lead"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg><span class="sr-only">Delete</span></button></form></div></td>';
            if (existingRow) {
