(function() {
  ...
  window.openContactEditModalById = function(contactId) {
    ...
    const contact = list.find(function(c) { return Number(c.id) === Number(contactId); });
    if (contact) {
      const root = document.querySelector('[x-data*="editingContact"]') || document.querySelector('.space-y-6[x-data]');
      if (root && root._x_dataStack && root._x_dataStack[0]) {
        root._x_dataStack[0].editingContact = Object.assign({}, contact);
  ...
  window.openEditWebsiteModalById = function(linkId) {
    ...
    const link = list.find(function(l) { return Number(l.id) === Number(linkId); });
    if (link) {
      const root = document.querySelector('[x-data*="editingWebsite"]') || document.querySelector('.space-y-6[x-data]');
      if (root && root._x_dataStack && root._x_dataStack[0]) {
        root._x_dataStack[0].editingWebsite = Object.assign({}, link);
        root._x_dataStack[0].editWebsiteModalOpen = true;
      } else {
        const rootEco = document.querySelector('[x-data*="editingLink"]');
        if (rootEco && rootEco._x_dataStack && rootEco._x_dataStack[0]) {
          rootEco._x_dataStack[0].editingLink = Object.assign({}, link);
  ...
  window.openPlacementModalById = function(parentId, branch, slotNumber) {
    ...
    const p = DATA.nodes.find(function(n) { return Number(n.id) === Number(parentId); });
    const pName = p ? p.member_name : '';
    const pCode = p ? (p.member_code || ('SBL-' + p.id)) : '';
    var c = document.querySelector('[x-data]');
    if (c && c._x_dataStack) {
      c._x_dataStack[0].openPlacementModal(parentId, pName, pCode, branch, slotNumber);
    }
  };

  window.openEditTaskById = function(taskId) {
    if (!DATA || !DATA.tasks) return;
    const task = DATA.tasks.find(function(t) { return Number(t.id) === Number(taskId); });
    if (task) {
      var c = document.querySelector('[x-data]');
      if (c && c._x_dataStack) {
  ...
  window.openContentCalendarEditById = function(itemId) {
    ...
    const item = DATA.contentItems.find(function(ci) { return Number(ci.id) === Number(itemId); });
    if (item) {
      var c = document.querySelector('[x-data]');
      if (c && c._x_dataStack) {
  ...
  function runSync() {
    ...
    if (curPath === '/leads' || curPath.startsWith('/leads?')) {
      ...
      function renderLeadRow(lead) {
        ...
        const callIconSvg = '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.12.96.35 1.9.69 2.79a2 2 0 01-.45 2.11L8.09 9.89a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.89.34 1.83.57 2.79.69A2 2 0 0122 16.92z"/></svg>';
        const waIconSvg = '<svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M12.04 2c-5.46 0-9.91 4.45-9.91 9.91 0 1.75.46 3.45 1.32 4.95L2.05 22l5.25-1.38c1.45.79 3.08 1.21 4.74 1.21 5.46 0 9.91-4.45 9.91-9.91 0-2.65-1.03-5.14-2.9-7.01A9.816 9.816 0 0012.04 2m.01 1.67c4.54 0 8.24 3.7 8.24 8.24 0 2.2-.86 4.27-2.42 5.82a8.196 8.196 0 01-5.82 2.42c-1.45 0-2.87-.38-4.12-1.11l-.3-.18-3.12.82.83-3.04-.19-.31a8.18 8.18 0 01-1.25-4.42c0-4.54 3.7-8.24 8.23-8.24m4.52 11.66c-.25.7-.72 1.29-1.37 1.63-.52.27-1.18.42-2.12.06-.94-.37-1.92-.99-2.73-1.8-.81-.81-1.43-1.79-1.8-2.73-.36-.94-.21-1.6.06-2.12.34-.65.93-1.12 1.63-1.37.22-.08.45-.04.62.1l1.3 1.6c.14.17.17.41.07.61l-.6 1.2c-.1.2-.06.45.1.61.62.62 1.36 1.12 2.19 1.48.2.09.43.05.57-.1l.98-.98c.18-.18.44-.22.66-.1l1.96.98c.22.11.35.34.33.59-.02.26-.14.5-.32.67z"/></svg>';
        const fbIconSvg = '<svg class="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>';

        const callBtn = '<a href="tel:' + escapeHtml(lead.mobile) + '" title="Call ' + escapeHtml(lead.mobile) + '" class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-emerald-50 hover:bg-emerald-600 text-emerald-600 hover:text-white transition-all shadow-2xs active:scale-95" aria-label="Call ' + escapeHtml(lead.mobile) + '">' + callIconSvg + '</a>';
        const waBtn = cleanWhatsapp ? ('<a href="https://wa.me/' + cleanWhatsapp + '" target="_blank" rel="noopener noreferrer" title="WhatsApp (' + escapeHtml(lead.whatsapp || lead.mobile) + ')" class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-emerald-50 hover:bg-emerald-600 text-emerald-600 hover:text-white transition-all shadow-2xs active:scale-95" aria-label="WhatsApp (' + escapeHtml(lead.whatsapp || lead.mobile) + ')">' + waIconSvg + '</a>') : ('<span class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-slate-100 text-slate-300" title="No WhatsApp">' + waIconSvg + '</span>');
        const fbBtn = fbUrl ? ('<a href="' + escapeHtml(fbUrl) + '" target="_blank" rel="noopener noreferrer" title="Facebook Profile" class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-blue-50 hover:bg-blue-600 text-blue-600 hover:text-white transition-all shadow-2xs active:scale-95" aria-label="Facebook Profile">' + fbIconSvg + '</a>') : ('<span class="inline-flex items-center justify-center w-6 h-6 rounded-md bg-slate-100 text-slate-300" title="No Facebook URL">' + fbIconSvg + '</span>');

        return '<td class="py-3.5 px-4">' +
          '<div class="flex items-center gap-3">' +
            (lead.photo ? ('<div class="w-9 h-9 rounded-xl bg-orange-100 flex items-center justify-center overflow-hidden flex-shrink-0 border border-orange-200/50"><img src="' + escapeHtml(lead.photo) + '" class="w-full h-full object-cover"></div>') : ('<div class="w-9 h-9 rounded-xl bg-orange-100 text-orange-700 font-bold flex items-center justify-center text-xs flex-shrink-0">' + initialLetter + '</div>')) +
            '<div>' +
              '<a href="/leads/' + lead.id + '" class="font-bold text-slate-900 hover:text-orange-600 text-sm block">' + escapeHtml(lead.name) + '</a>' +
              '<div class="text-[11px] text-slate-400">' + escapeHtml(lead.location || 'No location') + (lead.profession_or_business ? ' • ' + escapeHtml(lead.profession_or_business) : '') + '</div>' +
            '</div>' +
          '</div>' +
        '</td>' +
        '<td class="py-3.5 px-4 whitespace-nowrap">' +
          '<div class="font-semibold text-slate-900 text-xs font-mono tracking-wide">' + escapeHtml(lead.mobile) + '</div>' +
          '<div class="flex items-center gap-1.5 mt-1.5">' +
            callBtn + waBtn + fbBtn +
          '</div>' +
        '</td>' +
        '<td class="py-3.5 px-4 whitespace-nowrap">' +
          '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ' + stageClass + '">' + stageLabel + '</span>' +
        '</td>' +
        '<td class="py-3.5 px-4 whitespace-nowrap">' +
          (lead.next_action_at ? ('<div class="text-xs font-medium text-slate-700">' + escapeHtml(lead.next_action_type || 'Action') + '<br><span class="text-[11px] text-slate-400">' + escapeHtml(lead.next_action_at) + '</span></div>') : '<span class="text-[11px] text-amber-600 font-semibold bg-amber-50 px-2 py-0.5 rounded-md">Needs Next Action</span>') +
        '</td>' +
        '<td class="py-3.5 px-4 text-right">' +
          '<div class="flex items-center justify-end gap-1.5">' +
            '<a href="/leads/' + lead.id + '" class="w-8 h-8 rounded-lg flex items-center justify-center bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-900 transition-all active:scale-95 shadow-xs border border-slate-200/60" title="View Lead Profile" aria-label="View Lead Profile">' +
              '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>' +
              '<span class="sr-only">View</span>' +
            '</a>' +
            '<a href="/leads/' + lead.id + '/edit" class="w-8 h-8 rounded-lg flex items-center justify-center bg-orange-50 hover:bg-orange-100 text-orange-600 hover:text-orange-800 transition-all active:scale-95 shadow-xs border border-orange-200/60" title="Edit Lead" aria-label="Edit Lead">' +
      ...
      function renderMobileCard(lead) {
        ...
        const callBtn = '<a href="tel:' + escapeHtml(lead.mobile) + '" class="inline-flex items-center gap-1 text-emerald-600 font-semibold">' + callIconSvg + ' Call</a>';
        const waBtn = cleanWhatsapp ? ('<a href="https://wa.me/' + cleanWhatsapp + '" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-emerald-600 font-semibold">' + waIconSvg + ' WhatsApp</a>') : '';
        const fbBtn = fbUrl ? ('<a href="' + escapeHtml(fbUrl) + '" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1 text-blue-600 font-semibold">' + fbIconSvg + ' Facebook</a>') : '';

        return '<div class="flex items-start justify-between gap-2">' +
          '<div class="flex items-center gap-3">' +
            (lead.photo ? ('<div class="w-10 h-10 rounded-xl bg-orange-100 flex items-center justify-center overflow-hidden flex-shrink-0 shadow-xs border border-orange-200/50"><img src="' + escapeHtml(lead.photo) + '" class="w-full h-full object-cover"></div>') : ('<div class="w-10 h-10 rounded-xl bg-orange-100 text-orange-700 font-bold flex items-center justify-center text-sm flex-shrink-0 shadow-xs">' + initialLetter + '</div>')) +
            '<div>' +
              '<a href="/leads/' + lead.id + '" class="font-bold text-slate-900 hover:text-orange-600 text-sm block">' + escapeHtml(lead.name) + '</a>' +
              '<div class="text-xs text-slate-500">' + escapeHtml(lead.mobile) + '</div>' +
            '</div>' +
          '</div>' +
          '<span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold ' + stageClass + '">' + stageLabel + '</span>' +
        '</div>' +
      ...
      function renderKanbanCard(lead) {
        ...
        return '<div class="flex items-start justify-between gap-2">' +
          '<a href="/leads/' + lead.id + '" class="font-bold text-sm text-slate-900 hover:text-orange-600 truncate block">' + escapeHtml(lead.name) + '</a>' +
          '<span class="px-1.5 py-0.5 text-[10px] font-bold rounded-md border flex-shrink-0 ' + tempClass + '">' + tempLabel + '</span>' +
        '</div>' +
        '<div class="text-xs text-slate-500 mt-1 flex items-center justify-between">' +
          '<span>📞 ' + escapeHtml(lead.mobile) + '</span>' +
          '<span class="font-bold text-slate-700">' + (lead.score || 25) + ' pts</span>' +
        '</div>' +
        '<div class="mt-2 pt-2 border-t border-slate-100 flex items-center justify-between text-[11px]">' +
          '<div class="flex items-center gap-1.5">' +
            (lead.next_action_at ? ('<span class="font-medium text-slate-600">' + escapeHtml(lead.next_action_type || 'Action') + '</span>') : '<span class="text-amber-600 font-semibold">No Action</span>') +
          '</div>' +
          '<div class="flex items-center gap-1">' +
            '<a href="/leads/' + lead.id + '" class="text-slate-400 hover:text-slate-700 p-1 rounded hover:bg-slate-100 transition-colors" title="View Lead Profile">👁️</a>' +
            '<a href="/leads/' + lead.id + '/edit" class="text-slate-400 hover:text-orange-600 p-1 rounded hover:bg-orange-50 transition-colors" title="Edit Lead">✏️</a>' +
            '<form action="/leads/' + lead.id + '" method="POST" onsubmit="return confirm(\'Are you sure you want to delete this lead?\');" class="inline">' +
              '<input type="hidden" name="_method" value="DELETE">' +
      ...
      if (DATA.leads) {
        ...
        if (mobileStack) {
          ...
          if (DATA.leads.length === 0) {
            mobileStack.innerHTML = '<div class="py-12 text-center text-slate-400"><div class="text-base font-semibold text-slate-700">No leads found</div><a href="/leads/create" class="mt-3 inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-orange-600 text-white text-xs font-semibold">+ Add New Lead</a></div>';
          } else {
            DATA.leads.forEach(function(lead) {
              const card = document.createElement('div');
              card.setAttribute('data-lead-id', lead.id);
              card.className = 'p-4 space-y-3 hover:bg-slate-50/50 transition-colors';
        ...
        const kanbanContainers = document.querySelectorAll('.kanban-cards-container[data-stage]');
        if (kanbanContainers.length > 0) {
          kanbanContainers.forEach(function(col) {
            const stage = col.getAttribute('data-stage');
            col.innerHTML = '';
            const stageLeads = DATA.leads.filter(function(l) { return (l.stage || 'new') === stage; });
            
            const colHeader = document.querySelector('#kanban-col-' + stage + ' span.text-xs.font-bold.text-slate-500');
            if (colHeader) colHeader.textContent = stageLeads.length;

            stageLeads.forEach(function(lead) {
              const kCard = document.createElement('div');
              kCard.setAttribute('data-lead-id', lead.id);
              kCard.className = 'bg-white rounded-xl p-3.5 border border-slate-200/90 shadow-xs hover:border-orange-300 hover:shadow-md transition-all cursor-grab active:cursor-grabbing kanban-card';
    ...
    const isLeadShow = leadShowParts.length === 2 && leadShowParts[0] === 'leads' && !isNaN(Number(leadShowParts[1]));
    if (isLeadShow) {
      ...
      if (lead) {
        ...
        if (avatarEl) {
          if (lead.photo) {
            avatarEl.className = 'w-14 h-14 rounded-2xl bg-orange-100 text-orange-700 font-bold text-xl flex items-center justify-center flex-shrink-0 shadow-xs overflow-hidden border border-orange-200/50';
            avatarEl.innerHTML = '<img src="' + escapeHtml(lead.photo) + '" class="w-full h-full object-cover">';
          } else {
            avatarEl.className = 'w-14 h-14 rounded-2xl bg-orange-100 text-orange-700 font-bold text-xl flex items-center justify-center flex-shrink-0 shadow-xs';
    ...
    const isLeadEdit = leadShowParts.length === 3 && leadShowParts[0] === 'leads' && !isNaN(Number(leadShowParts[1])) && leadShowParts[2] === 'edit';
    if (isLeadEdit) {
    ...
    if (curPath === '/binary' || curPath.startsWith('/binary?') || curPath.startsWith('/binary/') || curPath === '/team' || curPath.startsWith('/team?') || curPath.startsWith('/team/')) {
      ...
      if (DATA.nodes && DATA.nodes.length > 0) {
        ...
        if ((bParts[0] === 'team' || bParts[0] === 'binary') && bParts[1] && !isNaN(Number(bParts[1]))) {
    ...
    if (curPath === '/contacts' || curPath.startsWith('/contacts?')) {
      ...
      if (contactsTbody && DATA.contacts && Array.isArray(DATA.contacts)) {
        contactsTbody.innerHTML = DATA.contacts.map(function(c) {
          const isPrimary = Number(c.is_primary) === 1;
          const iconBg = isPrimary ? 'bg-emerald-50 border border-emerald-200 text-emerald-700' : 'bg-slate-100 border border-slate-200/80 text-slate-700';
          const badgeHtml = c.badge ? ('<span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider ' + (isPrimary ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-slate-100 text-slate-700 border border-slate-200') + '">' + escapeHtml(c.badge) + '</span>') : '';
          const descHtml = c.description ? ('<p class="text-xs text-slate-500 line-clamp-1 mt-0.5 max-w-xs" title="' + escapeHtml(c.description) + '">' + escapeHtml(c.description) + '</p>') : '';
          const cleanPhone = (c.phone || '').replace(/[^0-9]/g, '');
          const cleanWa = (c.whatsapp || '').replace(/[^0-9]/g, '');

          let personHtml = '<span class="text-slate-400 text-xs">—</span>';
          if (c.contact_person) {
            personHtml = '<div class="flex items-center gap-2 text-sm text-slate-800 font-medium">' +
          ...
          if (c.phone) {
            phoneHtml = '<div class="flex items-center gap-2">' +
              ...
              '</button>' +
              '<a href="tel:' + cleanPhone + '" title="Direct Call" class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-slate-900 hover:bg-black text-emerald-400 shadow-xs transition-colors active:scale-95">' +
          ...
          if (c.whatsapp) {
            waHtml = '<div class="flex items-center gap-2">' +
              '<span class="font-bold text-slate-900 text-sm font-mono">' + escapeHtml(c.whatsapp) + '</span>' +
              '<button type="button" onclick="window.copyContactToClipboard(\' ' + escapeHtml(c.whatsapp) + '\', \'WhatsApp number\')" title="Copy WhatsApp" class="text-slate-400 hover:text-slate-700 p-1 rounded-md hover:bg-slate-100 transition-colors cursor-pointer">' +
                '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>' +
              '</button>' +
              '<a href="https://wa.me/' + cleanWa + '?text=' + encodeURIComponent('Hello, I would like to connect with SBL Helpdesk.') + '" target="_blank" rel="noopener noreferrer" title="Chat on WhatsApp" class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs transition-colors active:scale-95">' +
          ...
          if (c.email) {
            emailHtml = '<a href="mailto:' + escapeHtml(c.email) + '" class="text-xs text-slate-700 hover:text-emerald-600 font-medium truncate max-w-[160px] inline-flex items-center gap-1.5" title="' + escapeHtml(c.email) + '">' +
          ...
          const hoursHtml = '<span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200/80 text-slate-600 text-xs font-medium">' +
            '<svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>' +
            '<span>' + escapeHtml(c.available_hours || '10:00 AM - 08:00 PM') + '</span>' +
          '</span>';

          const searchData = (c.department + ' ' + (c.contact_person || '') + ' ' + c.phone + ' ' + (c.whatsapp || '') + ' ' + (c.email || '') + ' ' + (c.badge || '') + ' ' + (c.description || '')).toLowerCase();

          return '<tr data-contact-id="' + c.id + '" data-search="' + escapeHtml(searchData) + '" class="hover:bg-slate-50/75 transition-colors group">' +
              ...
              '<div class="flex items-center gap-3">' +
                ...
                '<div class="min-w-0">' +
                  '<div class="flex items-center gap-2 flex-wrap">' +
                    '<span class="font-bold text-slate-900 text-sm group-hover:text-emerald-700 transition-colors">' + escapeHtml(c.department) + '</span>' +
                    badgeHtml +
                  '</div>' +
                  descHtml +
                '</div>' +
      ...
      if (contactSearchInput && !contactSearchInput.__hasListener) {
        ...
        contactSearchInput.addEventListener('input', function(e) {
          ...
          const rows = document.querySelectorAll('#contacts-table-body tr[data-contact-id]');
    ...
    if (curPath === '/toolkit' || curPath.startsWith('/toolkit?') || curPath === '/ecosystem' || curPath.startsWith('/ecosystem?')) {
      const toolkitGrid = document.getElementById('toolkit-websites-grid');
      if (toolkitGrid && DATA.ecosystem && Array.isArray(DATA.ecosystem)) {
        toolkitGrid.innerHTML = DATA.ecosystem.map(function(el) {
          const badgeHtml = el.badge ? ('<span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-orange-100 text-orange-800">' + escapeHtml(el.badge) + '</span>') : '';
          return '<div data-link-id="' + el.id + '" class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-5 flex flex-col justify-between hover:shadow-md hover:border-orange-200 transition-all group">' +
            '<div class="space-y-3">' +
              '<div class="flex items-start justify-between gap-3">' +
                '<div class="flex items-center gap-3">' +
                  ...
                  '</div>' +
                  '<div>' +
                    '<h4 class="font-bold text-slate-900 text-sm group-hover:text-orange-600 transition-colors">' + escapeHtml(el.title) + '</h4>' +
                    '<span class="text-[10px] font-semibold text-slate-400">' + escapeHtml(el.category) + '</span>' +
                  '</div>' +
                '</div>' +
                badgeHtml +
              '</div>' +
              '<p class="text-xs text-slate-600 leading-relaxed">' + escapeHtml(el.description || '') + '</p>' +
              '<div class="text-[11px] font-mono text-slate-400 truncate">' + escapeHtml(el.url) + '</div>' +
            '</div>' +
            '<div class="pt-3 mt-3 border-t border-slate-100 flex items-center justify-between gap-2">' +
              '<a href="' + escapeHtml(el.url) + '" target="_blank" rel="noopener noreferrer" class="flex-1 px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold rounded-xl transition-colors flex items-center justify-center gap-1.5 shadow-xs">' +
                '<span>Visit Website</span>' +
                '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg>' +
              '</a>' +
              '<button type="button" onclick="window.openEditWebsiteModalById(' + el.id + ')" class="p-2 text-slate-400 hover:text-orange-600 hover:bg-orange-50 rounded-xl transition-colors cursor-pointer" title="Edit Website">✏️</button>' +
              '<form action="/ecosystem/' + el.id + '" method="POST" onsubmit="return confirm(\x27Remove ' + escapeHtml(el.title).replace(/'/g, "\\'") + '?\x27);" class="inline">' +
                '<input type="hidden" name="_method" value="DELETE">' +
